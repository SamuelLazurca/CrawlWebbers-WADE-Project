...
export const ConceptCard: React.FC<ConceptCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "group relative overflow-hidden rounded-2xl p-6 border backdrop-blur-sm transition-all duration-300",
        "bg-linear-to-br from-slate-800/50 to-slate-900/30 border-slate-700/50",
        onClick && "cursor-pointer hover:shadow-xl hover:scale-105 hover:-translate-y-1"
      )}
    >
      <div
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              void navigator.clipboard.writeText(concept.uri);
            }}
